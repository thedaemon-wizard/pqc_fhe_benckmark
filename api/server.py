...
- Multi-era Shor resource estimation (4 generations)
...
class QuantumThreatRequest(BaseModel):
    algorithm: str = Field("RSA-2048", description="Algorithm to assess (RSA-2048, ECC-256, AES-128, etc.)")
    key_size: int = Field(2048, description="Key size in bits")
    growth_model: str = Field("moderate", description="QPU growth model: conservative, moderate, aggressive")

class QuantumThreatResponse(BaseModel):
    algorithm: str
    key_size: int
    logical_qubits: int
    physical_qubits: int
...
class SecurityAssessmentRequest(BaseModel):
    inventory_template: str = Field("enterprise", description="Inventory template: enterprise, financial, government")
    org_profile: Optional[Dict[str, Any]] = Field(None, description="Organization profile")
...
class MPCHEDemoRequest(BaseModel):
    data: List[float] = Field(..., description="Input data from ALICE")
    fhe_mode: str = Field("gpu", description="FHE mode: gpu (default) or cpu")

class MPCHEDemoResponse(BaseModel):
    demo_type: str
    result: list
...
@app.post("/quantum/threat-assessment", response_model=QuantumThreatResponse, tags=["Quantum Threat"])
async def quantum_threat_assessment(request: QuantumThreatRequest):
    """Assess quantum threat to a specific algorithm."""
    ...
    try:
        if 'RSA' in algo:
            est = shor.estimate_rsa_resources(request.key_size)
        elif 'ECC' in algo or 'ECDSA' in algo or 'ECDH' in algo:
            est = shor.estimate_ecc_resources(request.key_size)
        elif 'DH' in algo:
            est = shor.estimate_dh_resources(request.key_size)
        elif 'AES' in algo:
            est = grover.estimate_aes_resources(request.key_size)
        elif 'SHA' in algo:
            est = grover.estimate_sha_resources(request.key_size)
        else:
            raise HTTPException(status_code=400, detail=f"Unknown algorithm: {request.algorithm}")

        return QuantumThreatResponse(
            algorithm=est.algorithm,
            key_size=est.key_size_bits,
            logical_qubits=est.logical_qubits,
            physical_qubits=est.physical_qubits,
            estimated_threat_year=est.estimated_threat_year,
            threat_level=est.threat_level,
            quantum_speedup=est.quantum_speedup_factor,
            attack_type=est.attack_type,
            classical_security=est.classical_bits_security,
            post_quantum_security=est.post_quantum_security,
            notes=est.notes,
        )
    except ValueError as e:
...
@app.get("/quantum/threat-timeline", response_model=ThreatTimelineResponse, tags=["Quantum Threat"])
async def quantum_threat_timeline(growth_model: str = "moderate"):
    """Generate comprehensive quantum threat timeline."""
    from src.quantum_threat_simulator import QuantumThreatTimeline

    try:
        timeline = QuantumThreatTimeline(growth_model)
...
@app.get("/quantum/pqc-comparison", tags=["Quantum Threat"])
async def quantum_pqc_comparison(growth_model: str = "moderate"):
    """Compare classical algorithm vulnerability vs PQC resilience."""
    from src.quantum_threat_simulator import QuantumThreatTimeline

    timeline = QuantumThreatTimeline(growth_model)
    return timeline.compare_classical_vs_pqc()


@app.get("/quantum/shor-simulation/{key_size}", tags=["Quantum Threat"])
async def shor_simulation(key_size: int = 2048, steps: int = 50):
    """Run educational Shor's algorithm simulation."""
    from src.quantum_threat_simulator import ShorSimulator

    shor = ShorSimulator()
    simulation = shor.simulate_factoring_progress(key_size, steps)
    return {
        'algorithm': f'RSA-{key_size}',
        'steps': [s.to_dict() for s in simulation],
        'total_steps': len(simulation),
    }


@app.get("/quantum/grover-simulation/{key_size}", tags=["Quantum Threat"])
async def grover_simulation(key_size: int = 128, steps: int = 50):
    """Run educational Grover's algorithm simulation."""
    from src.quantum_threat_simulator import GroverSimulator

    grover = GroverSimulator()
    simulation = grover.simulate_key_search(key_size, steps)
    return {
...
@app.post("/security/assess", response_model=SecurityScoreResponse, tags=["Security Scoring"])
async def security_assessment(request: SecurityAssessmentRequest):
    ...
    template_map = {
        'enterprise': engine.create_sample_enterprise_inventory,
        'financial': engine.create_sample_financial_inventory,
        'government': engine.create_sample_government_inventory,
    }

    factory = template_map.get(request.inventory_template)
    if not factory:
        raise HTTPException(status_code=400, detail=f"Unknown template: {request.inventory_template}")

    inventory = factory()
    org_profile = request.org_profile or {}
    score = engine.calculate_overall_score(inventory, org_profile)
    result = score.to_dict()

    return SecurityScoreResponse(**result)


@app.get("/security/compliance/{standard}", tags=["Security Scoring"])
async def security_compliance(standard: str):
    ...
    standard_map = {
        'nist_ir_8547': ComplianceStandard.NIST_IR_8547,
        'cnsa_2_0': ComplianceStandard.CNSA_2_0,
        'fips_140_3': ComplianceStandard.FIPS_140_3,
        'sp_800_57': ComplianceStandard.NIST_SP_800_57,
    }

    std = standard_map.get(standard)
    if not std:
        raise HTTPException(status_code=400, detail=f"Unknown standard: {standard}")

    checks = engine.check_compliance(std, inventory)
    return {
        'standard': standard,
...
@app.get("/security/migration-plan", tags=["Security Scoring"])
async def security_migration_plan(template: str = "enterprise"):
    ...
    factory = template_map.get(template, engine.create_sample_enterprise_inventory)
    inventory = factory()
...
@app.get("/security/inventory-templates", tags=["Security Scoring"])
async def security_inventory_templates():
    """List available inventory templates."""
    return {
        'templates': [
            {'name': 'enterprise', 'description': 'Typical enterprise IT environment'},
            {'name': 'financial', 'description': 'Financial services with high-sensitivity data'},
            {'name': 'government', 'description': 'Government/defense targeting CNSA 2.0'},
        ]
    }


# =============================================================================
# MPC-HE PRIVATE INFERENCE (v3.0.0)
# =============================================================================

@app.get("/mpc-he/protocol-info", tags=["MPC-HE"])
async def mpc_he_protocol_info():
    """Get MPC-HE protocol information and capabilities."""
    from src.mpc_he_inference import get_protocol_info
    return get_protocol_info()


@app.post("/mpc-he/demo/{demo_type}", tags=["MPC-HE"])
async def mpc_he_demo(demo_type: str, request: MPCHEDemoRequest):
    """Run an MPC-HE demonstration."""
    try:
        from src.mpc_he_inference import SimpleMPCDemo, MPCConfig
    except ImportError:
        raise HTTPException(status_code=503, detail="MPC-HE module not available")

    try:
        import desilofhe
    except ImportError:
        raise HTTPException(status_code=503, detail="desilofhe not installed")

    config = MPCConfig(
        fhe_mode=request.fhe_mode,
        use_bootstrap=False,
        log_n=14,
    )
    ...
    try:
        if demo_type == 'linear_regression':
            result = demo.run_linear_regression_demo(request.data)
        elif demo_type == 'classification':
            result = demo.run_classification_demo(request.data)
        elif demo_type == 'nn_inference':
            result = demo.run_nn_inference_demo(request.data)
        elif demo_type == 'private_prediction':
            result = demo.run_private_prediction_demo(request.data)
        elif demo_type == 'private_statistics':
            mid = len(request.data) // 2
            result = demo.run_private_statistics_demo(
                request.data[:mid], request.data[mid:]
            )
        else:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown demo: {demo_type}. Choose: linear_regression, classification, nn_inference, private_prediction, private_statistics"
            )

        return result
    except Exception as e:
        logger.error("MPC-HE demo failed: %s", e)
...
@app.post("/benchmarks/quantum-threat", tags=["Benchmarks"])
async def run_quantum_threat_benchmarks(iterations: int = 10):
    """Run quantum threat estimation benchmarks."""
    from benchmarks import benchmark_quantum_threat_estimation
    results = benchmark_quantum_threat_estimation(iterations=iterations)
    return {'results': [r.to_dict() for r in results], 'count': len(results)}


@app.post("/benchmarks/security-scoring", tags=["Benchmarks"])
async def run_security_scoring_benchmarks(iterations: int = 50):
    """Run security scoring benchmarks."""
    from benchmarks import benchmark_security_scoring
    results = benchmark_security_scoring(iterations=iterations)
    return {'results': [r.to_dict() for r in results], 'count': len(results)}


@app.post("/benchmarks/extended", tags=["Benchmarks"])
async def run_extended_benchmarks(
    iterations: int = 20,
    include_quantum_threat: bool = True,
    include_security_scoring: bool = True,
    include_mpc_he: bool = False,
    include_gpu: bool = True,
    include_security_perf: bool = True,
    fhe_mode: str = "gpu",
):
    """Run extended benchmark suite (v3.0.0)."""
    from benchmarks import run_extended_benchmark
    suite = run_extended_benchmark(
        iterations=iterations,
        include_quantum_threat=include_quantum_threat,
        include_security_scoring=include_security_scoring,
        include_mpc_he=include_mpc_he,
        include_gpu=include_gpu,
        include_security_perf=include_security_perf,
        fhe_mode=fhe_mode,
    )
...
# =============================================================================
# QUANTUM VERIFICATION (v3.1.0)
# =============================================================================

class ShorVerificationRequest(BaseModel):
    number: int = Field(15, description="Number to factor (15, 21, or 35)")
    shots: int = Field(4096, description="Number of circuit execution shots")
    base_a: Optional[int] = Field(None, description="Base for modular exponentiation")


class GroverVerificationRequest(BaseModel):
    num_qubits: int = Field(4, description="Search space size in qubits (3-20)")
    target: Optional[int] = Field(None, description="Target state (random if None)")
    shots: int = Field(4096, description="Number of circuit execution shots")


@app.post("/quantum/verify/shor", tags=["Quantum Verification"])
async def quantum_verify_shor(request: ShorVerificationRequest):
    """Run actual Shor's algorithm quantum circuit simulation to factor a small number."""
    from src.quantum_verification import ShorCircuitVerifier, QISKIT_AVAILABLE

    if not QISKIT_AVAILABLE:
        raise HTTPException(status_code=503, detail="Qiskit not installed. Run: pip install qiskit qiskit-aer")

    if request.number not in (15, 21, 35, 143, 221):
        raise HTTPException(status_code=400, detail="Supported numbers: 15, 21, 35, 143, 221")

    try:
        verifier = ShorCircuitVerifier(shots=request.shots)
        result = verifier.factor(request.number, a=request.base_a)
        return result.to_dict()
    except Exception as e:
        logger.error("Shor verification failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/quantum/verify/grover", tags=["Quantum Verification"])
async def quantum_verify_grover(request: GroverVerificationRequest):
    """Run actual Grover's search quantum circuit simulation."""
    from src.quantum_verification import GroverCircuitVerifier, QISKIT_AVAILABLE

    if not QISKIT_AVAILABLE:
        raise HTTPException(status_code=503, detail="Qiskit not installed. Run: pip install qiskit qiskit-aer")

    if not (3 <= request.num_qubits <= 20):
        raise HTTPException(status_code=400, detail="num_qubits must be between 3 and 20")

    try:
        verifier = GroverCircuitVerifier(shots=request.shots)
        result = verifier.search(request.num_qubits, target=request.target)
        return result.to_dict()
    except Exception as e:
        logger.error("Grover verification failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/quantum/verify/nist-levels", tags=["Quantum Verification"])
async def quantum_verify_nist_levels():
    """Verify NIST security levels for all PQC algorithms with lattice analysis."""
    ...
    try:
        ...
        return {
            ...
            'summary': {
                ...
                'algorithms_verified': [r.algorithm for r in results],
            },
            'generated_at': datetime.now().isoformat(),
        }
    except Exception as e:
        logger.error("NIST level verification failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


# =============================================================================
# SECTOR BENCHMARKS (v3.1.0)
# =============================================================================

@app.get("/benchmarks/sector/{sector}", tags=["Sector Benchmarks"])
async def run_sector_benchmark(sector: str, iterations: int = 20, fhe_mode: str = "gpu"):
    """Run sector-specific benchmarks (healthcare, finance, blockchain, iot, mpc-fhe)."""
    from src.sector_benchmarks import SectorBenchmarkRunner

    valid_sectors = ['healthcare', 'finance', 'blockchain', 'iot', 'mpc-fhe']
    if sector not in valid_sectors:
        raise HTTPException(status_code=400, detail=f"Unknown sector: {sector}. Valid: {valid_sectors}")

    runner = SectorBenchmarkRunner(fhe_mode=fhe_mode, iterations=iterations)
    sector_map = {
        'healthcare': runner.run_healthcare,
        'finance': runner.run_finance,
        'blockchain': runner.run_blockchain,
        'iot': runner.run_iot,
        'mpc-fhe': runner.run_mpc_fhe,
    }

    try:
        result = sector_map[sector]()
        return result.to_dict()
    except Exception as e:
        logger.error("Sector benchmark failed (%s): %s", sector, e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/benchmarks/sector-all", tags=["Sector Benchmarks"])
async def run_all_sector_benchmarks(iterations: int = 10, fhe_mode: str = "gpu"):
    ...
    try:
        ...
        return {sector: suite.to_dict() for sector, suite in results.items()}
    except Exception as e:
        logger.error("All-sector benchmarks failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))
...
@app.get("/quantum/shor-resources/multi-era", tags=["Quantum Verification"])
async def shor_multi_era_resources(key_size: int = 2048):
    """
    Compare Shor's algorithm resource estimates across four generations (2021-2026).

    Shows the dramatic reduction in physical qubit requirements from 20M (2021)
    to 100K (2026) due to advances in QLDPC codes and magic state cultivation.
    """
    try:
        from src.quantum_threat_simulator import ShorSimulator
        shor = ShorSimulator()
        return shor.estimate_rsa_resources_multi_era(key_size)
    except Exception as e:
        logger.error("Multi-era Shor estimation failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/quantum/simulate/noisy", tags=["Quantum Verification"])
async def noisy_quantum_simulation(
    num_qubits: int = 4,
    circuit_type: str = "grover",
    error_rates: Optional[List[float]] = None,
):
    """
    Run noise-aware quantum circuit simulation.

    Compares ideal vs noisy execution at different error rates using
    depolarizing noise channels. Demonstrates why quantum error correction
    is essential for cryptographic applications.
    """
    try:
        from src.quantum_verification import NoiseAwareQuantumSimulator
        sim = NoiseAwareQuantumSimulator()
        return sim.compare_ideal_vs_noisy(circuit_type, num_qubits, error_rates)
    except ImportError as e:
        raise HTTPException(
            status_code=503,
            detail=f"Qiskit not available: {e}"
        )
    except Exception as e:
        logger.error("Noisy simulation failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/security/algorithm-diversity", tags=["Security Scoring"])
async def algorithm_diversity_assessment(template: str = "enterprise"):
    """
    Assess PQC algorithm family diversity to detect lattice monoculture risk.

    A deployment using only lattice-based PQC (ML-KEM + ML-DSA) has a single
    point of failure. This endpoint evaluates diversity across lattice, hash-based,
    and code-based algorithm families.

    Returns diversity score (0-100), family breakdown, and recommendations.
    """
    from src.security_scoring import SecurityScoringEngine
    ...
    factory = template_map.get(template)
    if not factory:
        raise HTTPException(status_code=400, detail=f"Unknown template: {template}")

    inventory = factory()
    return engine.assess_algorithm_diversity(inventory)


@app.get("/security/cnsa-readiness", tags=["Security Scoring"])
async def cnsa_readiness_assessment(template: str = "enterprise"):
    """
    ...
    Returns phase-by-phase compliance status, algorithm gaps, and timeline.
    """
    from src.security_scoring import SecurityScoringEngine
    ...
    org_profile = {
        ...
        'has_migration_plan': template == 'government',
        'has_hybrid_deployment': template in ('financial', 'government'),
    }
    return engine.assess_cnsa_2_0_readiness(inventory, org_profile)


@app.get("/security/masking-verification", tags=["Security Scoring"])
async def masking_verification(impl: str = "liboqs"):
    """
    Verify masking countermeasure status for all PQC algorithms.

    Checks whether the specified implementation provides masking
    protection against power analysis (SPA) and other side-channel attacks.

    Key finding: ML-KEM on liboqs lacks SPA protection (Berzati 2025:
    key recovery in 30 seconds on Cortex-M4).
    """
    try:
        from src.side_channel_assessment import SideChannelRiskAssessment
        sa = SideChannelRiskAssessment()
        return sa.verify_masking_deployment(impl=impl)
    except Exception as e:
        logger.error("Masking verification failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/security/side-channel/all", tags=["Security Scoring"])
async def side_channel_assessment_all():
    """
    Assess side-channel risks for all supported PQC algorithms.

    Returns comprehensive assessment with vulnerabilities, mitigations,
    and implementation hardening status for ML-KEM, ML-DSA, and SLH-DSA.
    """
    try:
        from src.side_channel_assessment import SideChannelRiskAssessment
        sa = SideChannelRiskAssessment()
        return sa.assess_all()
    except Exception as e:
        logger.error("Side-channel assessment (all) failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/security/side-channel/{algorithm}", tags=["Security Scoring"])
async def side_channel_assessment(algorithm: str):
    """
    Assess side-channel risks for a specific PQC algorithm.

    Returns known vulnerabilities, risk level, and mitigation recommendations
    based on latest research (2024-2026).

    Supported algorithms: ML-KEM, ML-DSA, SLH-DSA (and variants)
    """
    try:
        from src.side_channel_assessment import SideChannelRiskAssessment
        sa = SideChannelRiskAssessment()
        profile = sa.assess_algorithm(algorithm)
        return profile.to_dict()
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error("Side-channel assessment failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


# =============================================================================
# CKKS / FHE QUANTUM SECURITY ENDPOINTS (v3.2.0)
# =============================================================================

@app.get("/quantum/ckks-security", tags=["Quantum Verification"])
async def ckks_security_verification(
    log_n: int = 15, max_levels: int = 20, scale_bits: int = 40
):
    """
    Verify quantum security of CKKS FHE parameters.

    Assesses Ring-LWE security of CKKS configuration against HE Standard
    bounds and maps to NIST security levels. Identifies lattice monoculture
    risk (shared with ML-KEM/ML-DSA).

    Default: log_n=15, max_levels=20, scale_bits=40 (128-bit secure)
    """
    try:
        from src.quantum_verification import CKKSSecurityVerifier
        verifier = CKKSSecurityVerifier()
        return verifier.verify_ckks_config(
            log_n=max(10, min(log_n, 17)),
            max_levels=max(1, min(max_levels, 60)),
            scale_bits=max(20, min(scale_bits, 60)),
            config_name=f'API(log_n={log_n}, levels={max_levels})',
        )
    except Exception as e:
        logger.error("CKKS security verification failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/quantum/ckks-security/all-configs", tags=["Quantum Verification"])
async def ckks_security_all_configs():
    """
    Verify quantum security of all predefined CKKS configurations.

    Returns security assessment for Light, Medium, Standard, Heavy configs
    plus MPC-HE default/linear/NN configurations. Includes business impact
    analysis per sector and lattice monoculture warnings.
    """
    try:
        from src.quantum_verification import CKKSSecurityVerifier
        verifier = CKKSSecurityVerifier()
        return verifier.verify_all_configs()
    except Exception as e:
        logger.error("CKKS all-config verification failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/security/fhe-quantum-risk", tags=["Security Scoring"])
async def fhe_quantum_risk(log_n: int = 15, max_levels: int = 20):
    """
    Assess FHE deployment quantum risk in context of overall PQC security posture.

    Evaluates how CKKS Ring-LWE hardness interacts with ML-KEM/ML-DSA lattice
    security. Returns risk score (0-100), risk factors, and diversification strategy.
    """
    try:
        from src.security_scoring import SecurityScoringEngine
        engine = SecurityScoringEngine()
        return engine.assess_fhe_quantum_security(
            log_n=max(10, min(log_n, 17)),
            max_levels=max(1, min(max_levels, 60)),
        )
    except Exception as e:
        logger.error("FHE quantum risk assessment failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


# =============================================================================
# GL SCHEME ENDPOINTS (v3.2.0)
# =============================================================================

@app.get("/fhe/gl-scheme/info", tags=["FHE"])
async def gl_scheme_info():
    """
    Get GL scheme (5th generation FHE) capabilities and status.

    Returns supported matrix shapes, operations, security assessment,
    and known vulnerabilities. GL provides native matrix multiplication
    in O(1) homomorphic operations vs CKKS O(n) rotations.

    Reference: ePrint 2025/1935 (Gentry & Lee)
    """
    try:
        from src.gl_scheme_engine import get_gl_scheme_info
        return get_gl_scheme_info()
    except Exception as e:
        logger.error("GL scheme info failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/fhe/gl-scheme/security", tags=["FHE"])
async def gl_scheme_security():
    """
    Get GL scheme security information and known vulnerabilities.

    Includes NTT SPA risk (shared with CKKS), CPAD risk for threshold
    variants, advantages over CKKS, and current limitations.

    Reference:
    - ePrint 2025/1935 (GL scheme)
    - arXiv:2505.11058 (NTT SPA)
    - CEA 2025 (CPAD attack)
    """
    try:
        from src.gl_scheme_engine import GLSchemeEngine, GLConfig
        engine = GLSchemeEngine(GLConfig(shape=(256, 16, 16)))
        return engine.get_security_info()
    except Exception as e:
        logger.error("GL scheme security info failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/mpc-he/gl-inference/info", tags=["MPC-HE"])
async def gl_private_inference_info():
    """
    Get GL scheme private inference capabilities.

    Returns information about GL-based 2-party private inference,
    supported operations, security notes, and comparison with CKKS.

    Reference:
    - ePrint 2025/1935 (GL scheme)
    - RhombusEnd2End_HEonGPU (GPU 2-party inference)
    - DESILO FHE GLEngine API
    """
    try:
        ...
        logger.error("GL inference info failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))
...
def main():
    ...
    print("Noisy Simulation: POST http://127.0.0.1:8000/quantum/simulate/noisy")
    print("Side-Channel: http://127.0.0.1:8000/security/side-channel/all")
    print("Algo Diversity: http://127.0.0.1:8000/security/algorithm-diversity")
    print("CNSA Readiness: http://127.0.0.1:8000/security/cnsa-readiness")
    print("Masking Check: http://127.0.0.1:8000/security/masking-verification")
    print("CKKS Security: http://127.0.0.1:8000/quantum/ckks-security")
    print("CKKS All Configs: http://127.0.0.1:8000/quantum/ckks-security/all-configs")
    print("FHE Quantum Risk: http://127.0.0.1:8000/security/fhe-quantum-risk")
    print("Security Scoring: http://127.0.0.1:8000/security/assess")
    print("MPC-HE Info: http://127.0.0.1:8000/mpc-he/protocol-info")
